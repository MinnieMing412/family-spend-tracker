"""Deterministic parser for supported Bank of America credit-card statements."""
...
_TRANSACTION_START = re.compile(
    rf"^\s*(?P<transaction_date>{_DATE_TOKEN})\s+"
    rf"(?:(?P<posting_date>{_DATE_TOKEN})\s+)?(?P<body>.+?)\s*$",
    re.IGNORECASE,
)
_AMOUNT_AT_END = re.compile(
    rf"^(?P<description>.*?)\s+(?P<amount>{_AMOUNT_TOKEN})\s*$",
    re.IGNORECASE,
)
_TOTAL_LINE = re.compile(
    rf"^\s*(?P<label>Payments\s+and\s+Other\s+Credits|Purchases\s+and\s+Adjustments|"
    rf"Fees\s+Charged|Interest\s+Charged)\s+(?P<amount>{_AMOUNT_TOKEN})\s*$",
    re.IGNORECASE,
)
_BILLING_PERIOD_PATTERNS = (
    re.compile(
        rf"(?:Billing\s+Cycle|Statement\s+Period)\s*:?\s*(?P<start>{_DATE_TOKEN})\s*"
        rf"(?:-|through|to)\s*(?P<end>{_DATE_TOKEN})",
        re.IGNORECASE,
    ),
    re.compile(
        rf"(?P<start>{_DATE_TOKEN})\s*(?:-|through|to)\s*(?P<end>{_DATE_TOKEN})\s*"
        r"(?:Billing\s+Cycle|Statement\s+Period)",
        re.IGNORECASE,
    ),
)
...
_ACCOUNT_LINE = re.compile(
    r"(?:Account(?:\s+Number)?|Acct)\s*(?:Ending\s+in)?\s*[:#]?\s*(?P<value>[^\n]+)",
    re.IGNORECASE,
)
_CARDHOLDER = re.compile(
    r"^\s*(?:Cardholder|Cardmember)\s*:\s*(?P<name>.+?)"
    r"(?:\s+Account\s+(?:Number\s*)?[:#].*)?$",
    re.IGNORECASE,
)
_TRAILING_REFERENCE = re.compile(
    r"^(?P<description>.+?)\s+(?P<reference>[A-Z0-9]{8,23})\s+(?P<account>\d{4})$",
    re.IGNORECASE,
)
...
class BankOfAmericaStatementParser:
    ...
    def parse(self, source: ValidatedPdf) -> ParseResult:
        ...
        for page_number, page_text in enumerate(source.page_texts, start=1):
            for line_number, raw_line in enumerate(page_text.splitlines(), start=1):
                ...
                cardholder_match = _CARDHOLDER.match(line)
                ...
                if total_match is not None and section == "other":
                ...
                if cardholder_match is not None:
                    cardholder = " ".join(cardholder_match.group("name").split()).upper()
                    continue
        ...
        if not transactions:
            raise FamilySpendError(
                "Unsupported Bank of America credit-card layout: "
        ...
        if not totals:
            warnings.append(
                DomainWarning(
                    code="boa-reported-totals-missing",
                    message="No Bank of America account-summary totals could be extracted.",
    ...
    def _account_id(text: str, source_name: str) -> str:
        match = _ACCOUNT_LINE.search(text)
        if match is None:
            raise FamilySpendError(
                "Unsupported Bank of America credit-card layout: "
    ...
    def _billing_period(cls, text: str, source_name: str) -> tuple[date, date]:
        ...
        raise FamilySpendError(
            "Unsupported Bank of America credit-card layout: "
            f"billing cycle is missing in {source_name}",
            2,
        )
    ...
    def _section_for(line: str) -> str | None:
        normalized = " ".join(line.casefold().replace("&", "and").split())
        normalized = normalized.removesuffix(" continued").strip(":")
        if normalized == "payments and other credits":
            return "payments_and_other_credits"
        if normalized in {"purchases and adjustments", "purchases"}:
            return "new_charges"
        if normalized in {"balance transfers", "balance transfer"}:
            return "balance_transfers"
        if normalized in {"cash advances", "cash advance"}:
    ...
    def _build_total(cls, match: re.Match[str]) -> StatementTotal:
        normalized = "_".join(match.group("label").casefold().split())
        section = {
            "payments_and_other_credits": "payments_and_other_credits",
            "purchases_and_adjustments": "new_charges",
            "fees_charged": "fees",
            "interest_charged": "interest_charged",
        }[normalized]
        amount = cls._parse_amount(match.group("amount"))
        amount = -abs(amount) if section == "payments_and_other_credits" else abs(amount)
        return StatementTotal(section, Money(amount))
    ...
    def _build_transaction(
        ...
        description, reference, row_account = cls._description_parts(description)
        ...
        if reference:
            metadata.append(("reference_suffix", reference[-6:]))
        if row_account:
            metadata.append(("row_account", f"ending-{row_account}"))
        stable_key = f"{source.sha256}:{index}:{pending.evidence_ref}"
        transaction_id = f"txn-{hashlib.sha256(stable_key.encode()).hexdigest()[:20]}"
        included_in_spend = transaction_type not in {
    ...
    @staticmethod
    def _description_parts(value: str) -> tuple[str, str | None, str | None]:
        match = _TRAILING_REFERENCE.match(value)
        if match is None:
    ...
    def _parse_amount(value: str) -> Decimal:
        ...
        normalized = normalized.removesuffix("CR").strip().removesuffix("-")
        normalized = normalized.replace("$", "").replace(",", "").removeprefix("-")
        ...
        try:
            ...
            raise FamilySpendError(
                "Bank of America statement contains an invalid amount",
                2,
            ) from error
    ...
    def _parse_date(value: str, reference: date | None) -> date:
        ...
        if reference is None:
            raise FamilySpendError("Bank of America statement date is missing a year", 2)
        month, day = pieces
        year = reference.year
    ...
    def _incomplete_warning(pending: _PendingTransaction) -> DomainWarning:
        return DomainWarning(
            code="boa-partial-transaction-row",
            message="A Bank of America transaction row did not contain a parseable amount.",
            severity=WarningSeverity.WARNING,
            evidence_ref=pending.evidence_ref,
        )
